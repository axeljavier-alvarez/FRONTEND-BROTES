...
@Component({
  selector: 'app-historialclienteadmin',
  templateUrl: './historialclienteadmin.component.html',
  styleUrls: ['./historialclienteadmin.component.scss'],
  providers: [UsuarioService, AdminUsuariosService]
})
export class HistorialclienteadminComponent implements OnInit {
...
public HistorialModelPost: Historial;
 ...
 ngOnInit(): void {
  ...
  getHistorialPorPlanta(idPlanta) {
    this.
      _adminUsuarioService.obtenerHistorialPorIdPlanta(idPlanta, this.token)
      .subscribe(
        (response) => {
    ...
    this._adminUsuarioService.agregarHistorial(
  ...
  getHistorialId(idHistorial: string) {
    ...
    this._adminUsuarioService.verHistorialPorId(idHistorial, this.token).subscribe(
      (response) => {
  ...
  verHistorialPorId(idPlanta) {
    this._adminUsuarioService.verHistorialPorId(idPlanta, this.token).subscribe(
      (response) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      this._adminUsuarioService.editarHistorial(this.HistorialModelGetId, this.token).subscribe(
        (response) => {
          console.log(response);

          // ✅ ACTUALIZA EL ARREGLO EN EL FRONTEND
  ...
  deleteHistorial(idHistorial: string) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._adminUsuarioService.eliminarHistorial(idHistorial, this.token).subscribe(
          (response) => {
            console.log(response);
